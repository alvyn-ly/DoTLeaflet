...
define(['JQuery', 'JQuery_ui', 'leaflet', 'Leaflet_Google', 'leafletLib'], function(JQuery, leafletLib) {
	...
	init = function initializeMap(options) {
		...
		if (options.fastCustomLayer != null){
			for (var j = 0; j < options.fastCustomLayer.length; j++){
				createFastMarkerLayer(options.fastCustomLayer[j]);
			}
        ...
        if (options.customLayer != null){
            for (var j = 0; j < options.customLayer.length; j++){
                createMarkerLayer(options.customLayer[j], j);
                menu.addOverlay(layerArray[j], options.customLayer[j].name);
            }
        }
        ...
        function sfLayerHelper(number){
            var min = 0;
            var max = 20;
            if (options.fastCustomLayer[number].minZoom >= 0 && options.fastCustomLayer[number].maxZoom >= options.fastCustomLayer[number].minZoom){
                min = options.fastCustomLayer[number].minZoom;
                max = options.fastCustomLayer[number].maxZoom;
            }
    ...
    function getShops(){
    	...
    	for (var i = 0; i < recordsShop.length; i++){
            if (recordsShop[i].Geolocation__Latitude__s != null && recordsShop[i].Geolocation__Longitude__s != null){
              myIcon = L.icon({
              ...
              .bindPopup("Shop Number: " + recordsShop[i].Name + "<br>" + recordsShop[i].IntersectionName1__c)
              .on('click', function () {
						//this.bounce(3);
						try {
    ...
    function makeIcon(type, status, focus){
    	...
    	} else if (type == "Pedestrian Safety Enhancement") {
            pic = signalsPSEMarker;
        } else if (type == "Pavement") {
            pic = signalsPavementMarker;
        } else {
    ...
    function createFastMarkerLayer(records){
        ...
        var iconx = 25;
        var icony = 25;
        if (records.iconX > 0 && records.iconY > 0){
            iconx = records.iconX;
            icony = records.iconY;
        }
        for (var i = 0; i < records.items.length; i++){
            ...
            if (!records.items[i].Geolocation__Latitude__s || 0 === records.items[i].Geolocation__Latitude__s.length || !records.items[i].Geolocation__Longitude__s || 0 === records.items[i].Geolocation__Longitude__s.length) { continue; }
                ...
                icon = L.divIcon({
                    ...
                    className: 'leaflet-div-focusIcon',
                    iconSize: new L.Point(icony, iconx),
                    iconAnchor: new L.Point(icony/2, iconx),
                    popupAnchor: new L.Point(0, -iconx)
                });
    ...
    function createMarkerLayer(records, count){
        customMarker = L.Marker.extend({
            options: { 
                allData: "",
            }
        ...
        for (var i = 0; i < records.items.length; i++){
            var tempLat;
            var tempLng;
            if (records.items[i].Geolocation__Latitude__s && 0 != records.items[i].Geolocation__Latitude__s.length && records.items[i].Geolocation__Longitude__s && 0 != records.items[i].Geolocation__Longitude__s.length){
                tempLat = records.items[i].Geolocation__Latitude__s;
                tempLng = records.items[i].Geolocation__Longitude__s;
            } else if (records.items[i].Latitude__c && 0 != records.items[i].Latitude__c.length && records.items[i].Longitude__c && 0 != records.items[i].Longitude__c.length){
                tempLat = records.items[i].Latitude__c;
                tempLng = records.items[i].Longitude__c;
            }
            if (!tempLat || 0 === tempLat.length || !tempLng || 0 === tempLng.length) { continue; }
            else if (records.items[i].Name === options.focus){
                icon = L.divIcon({
                    html: records.focusImage,
                    className: 'leaflet-div-focusIcon',
                ...
                map.panTo(new L.LatLng(tempLat, tempLng));
            } else {
                icon = L.divIcon({
                    html: records.image,
                    // html: signalsProjMarker + orangeHue,
            ...
            var marker = new customMarker([tempLat, tempLng], {icon: icon, allData: records.items[i]})
            .bindPopup(L.Util.template(records.textBubble, records.items[i]))
            .on('click', function () {
                try {
                    pushData(this.options.allData);
        ...
        layerArray.push(L.layerGroup(newLayer));

        layerArray[count].addTo(map);
    ...
    function createProjMarkers(records){
        ...
        for (var i = 0; i < records.length; i++){
            if (records[i].Geolocation__Latitude__s != null && records[i].Geolocation__Longitude__s != null){
                ...
                .bindPopup("<p style=\"text-align:center\"><a href=\"../" + records[i].Id + "\" target=\"_blank\">" + "<b>" + records[i].Name + "</b>" + "</a><br>" + records[i].Name__c + "<br>" + records[i].StandardizedLocation__c + "<br>" + records[i].Status__c + "</p>")
                .on('click', function () {
                //this.bounce(3);
                try {
                    pushData(this.options.allData);
	 			...
	 			j$(searchbox).keypress(function(e) {
					if(e.which == 13) { //if a number is entered into the searchbox, it will attempt to find the shopNo instead. 
						...
						if (searchbox.value[0] == '#'){
							...
							// geocoder.geocode( {
							// 	address: shopRecords1[0].IntersectionName1__c + ", San Jose California",
							// 	bounds: bounds,
							// 	region: 'us'
							// }, function(results, status) {
							// 	if (status == google.maps.GeocoderStatus.OK) {
                               createDragMarker(shopRecords1[0].Geolocation__Latitude__s, shopRecords1[0].Geolocation__Longitude__s);
                               processLocation(shopRecords1[0].Geolocation__Longitude__s, shopRecords1[0].Geolocation__Latitude__s, toTitleCase(shopRecords1[0].IntersectionName1__c) , null, shopNum);
							// 	} else {
