...
define(['JQuery', 'JQuery_ui', 'leaflet', 'Leaflet_Google', 'leafletLib'], function(JQuery, leafletLib) {
	...
	init = function initializeMap(options) {
		...
		if (options.shopsLayer){
			...
			oms.addListener('spiderfy', function(shopArray) {
				map.closePopup();
			});
		}
    ...
    function getShops(){
    	...
    	for (var i = 0; i < recordsShop.length; i++){
    		myIcon = L.icon({
    			iconUrl: 'https://i.imgur.com/juXmwnf.png',
    			iconSize: [30, 30], 
    			iconAnchor: [15, 30], 
    			popupAnchor: [0, -30]  
    		});
    		...
    		.bindPopup(recordsShop[i].Name + "<br>" + recordsShop[i].IntersectionName__c)
    		.on('click', function () {
						//this.bounce(3);
						try {
    ...
    function placeMarkersInBounds() {
    	...
    	for (var i = shopArray.length -1; i >= 0; i--) {
    		var m = shopArray[i];
    		var shouldBeVisible = mapBounds.contains(m.getLatLng());
    		if ((m._icon && !shouldBeVisible) || map.getZoom() < 16) {
    			map.removeLayer(m);
    		} else if (!m._icon && shouldBeVisible && map.getZoom() >= 16) {
    			map.addLayer(m);
    		}
    	}
    }
	 ...
	 */
	 var addMarkers = function () {
		...
		div2.setAttribute('src', 'https://i.imgur.com/asjsp66.png');
